...
/*	actions[TRANSFORM_SUPER]->setText(tr("Make Superstable"));
	...
	actions[VIEW_FULLSCREEN]->setText(tr("Fullscreen"));
...
/*	append(MENU_FILE, FILE_LOAD, NULL);
	...
	append(MENU_FILE, FILE_QUIT, SLOT(close()), QKeySequence(Qt::CTRL + Qt::Key_Q));
/*	append(MENU_TRANSFORM, TRANSFORM_SUPER, NULL);
	...
	append(MENU_VIEW, VIEW_FULLSCREEN, SLOT(slot_fullscreen()), QKeySequence(Qt::CTRL + Qt::Key_F));
	append(MENU_HELP, HELP_ABOUT, SLOT(slot_help_about()));
	append(MENU_HELP, HELP_ABOUTQT, SLOT(slot_help_about_qt()));
	actions[VIEW_FULLSCREEN]->setCheckable(true);
